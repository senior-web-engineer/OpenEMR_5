...
switch ($api){
    ...
    case "saveproblemprimary": 
		...
		sqlStatement("UPDATE form_treatment_plan_problems SET IsPrimary = ". add_escape_custom(getPost("IsPrimary",0)) .  " WHERE id = ". add_escape_custom("$id"). "");
	...
	case "savemodalities":
		$sets = "pid = {$_SESSION["pid"]},
			form_id = " . add_escape_custom(getPost("form_id",0)) .  ", 
			user = '" . $_SESSION["authUser"] . "',
			start_date = '" . add_escape_custom(getPost("start_date",'')) .  "',
			end_date = '" . add_escape_custom(getPost("end_date",'')) .  "',
			modality = '" . add_escape_custom(getPost("modality",'')) .  "', 
			hcpt = '" . add_escape_custom(getPost("hcpt",'')) .  "', 
			intervals = " . add_escape_custom(getPost("intervals",0)) .  ", 
			frequency = '" . add_escape_custom(getPost("frequency",'')) .  "', 
			duration_hour = " . add_escape_custom(getPost("duration_hour",0)) .  ",
			duration_minute = " . add_escape_custom(getPost("duration_minute",0)) .  ",
			Description = '" . add_escape_custom(getPost("Description",'')) .  "', 
			provider = '" . add_escape_custom(getPost("provider",'')) .  "',
			tp_problem_number = " . add_escape_custom(getPost("tp_problem_number",0)) .  ",
			IsDeleted = " . add_escape_custom(getPost("IsDeleted",0)) .  ",
			problem_id = " . add_escape_custom(getPost("problem_id",0)) .  "
			";
		if (empty($id) || $id == -1) {
		  $id = sqlInsert("INSERT INTO form_treatment_plan_modalities SET $sets ");
		} else {
		  sqlStatement("UPDATE form_treatment_plan_modalities SET $sets WHERE id = ". add_escape_custom("$id"). "");
		}
		$output = "{\"id\": \"$id\"}";
		break;
