...
$query = "SELECT SQL_CALC_FOUND_ROWS {$selectQuery} FROM request_features WHERE user_id='{$authUserID}' LIMIT ?, ?";
...
if (isset($_GET['filterscount'])) {
  ...
  if ($filterscount > 0)
    {
    $where = " WHERE user_id='{$authUserID}' AND (";
    $tmpdatafield = "";
    $tmpfilteroperator = "";
    $valuesPrep = "";
    $value = [];
    ...
    $filterquery.= "SELECT SQL_CALC_FOUND_ROWS {$selectQuery} FROM request_features " . $where;
    // build the query.
    $valuesPrep = $valuesPrep . "ii";
    $values[] = & $start;
    $values[] = & $pagesize;
    $query = "SELECT SQL_CALC_FOUND_ROWS {$selectQuery} FROM request_features " . $where . " LIMIT ?, ?";    
    $result = $mysqli->prepare($query);
...
if (isset($_GET['sortdatafield']))
  ...
  if ($sortorder != '') {
    if ($_GET['filterscount'] == 0) {
      if ($sortorder == "desc") {
        $query = "SELECT SQL_CALC_FOUND_ROWS {$selectQuery} FROM request_features WHERE user_id='{$authUserID}' ORDER BY" . " " . $sortfield . " DESC LIMIT ?, ?";
      }
      else if ($sortorder == "asc") {
        $query = "SELECT SQL_CALC_FOUND_ROWS {$selectQuery} FROM request_features WHERE user_id='{$authUserID}' ORDER BY" . " " . $sortfield . " ASC LIMIT ?, ?";
      }
      $result = $mysqli->prepare($query);
      $result->bind_param('ii', $start, $pagesize);
    }
