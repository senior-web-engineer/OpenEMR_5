...
<thead> 
	<tr>
		...
		<th class="filter-select">Hours</th>
	</tr>
</thead>
	<tbody>
...
$count = $row['COUNT(pd.pid)'];
	...
	//echo $query_4;
	$query_4 = "SELECT SEC_TO_TIME(SUM(diffSeconds)) as total_time ".
				"FROM (".
				"SELECT ".
						"TIME_TO_SEC(".
						"TIMEDIFF(STR_TO_DATE(fm.time_end, '%h:%i%p'), STR_TO_DATE(fm.time_start, '%h:%i%p'))".
		") as diffSeconds ".
				"FROM forms AS f ".
				//"JOIN form_encounter AS e ON e.id = f.form_id ".
				//"AND e.encounter = f.encounter ".
