...
<th>Description</th> 
	<th  style="width: 109px">Result</th> </tr></thead><tbody>
...
//$query = "SELECT ".
//" pd.id, pd.fname, pd.lname, pd.mname, pd.sex, pd.patient_active, pd.providerID, pd.group_providerID, prd.id AS prid, prd.fname AS pfname, prd.lname AS plname, grp.id AS grpid, grp.fname AS grpfname, grp.lname AS grplname, max(en.date) AS ldate ".
// "FROM  `patient_data` AS pd ". 
// " LEFT JOIN users AS prd ON prd.id = pd.providerID ".
// " LEFT JOIN users AS grp ON grp.id = pd.group_providerID ".
// " LEFT JOIN form_encounter AS en ON en.pid = pd.id ".
// "GROUP BY pd.id ".
// "ORDER BY pd.providerID ASC";
 $con = mysql_connect($host, $login, $pass); 
mysql_select_db($dbase, $con);
$query = "SELECT ".
 "COUNT(pd.pid), pd.pid, pd.patient_active ".
 "FROM  `patient_data` AS pd ".
 "WHERE patient_active = 'YES' "
 //"GROUP BY p.id"
 ;
...
$count = $row['COUNT(pd.pid)'];
	...
	echo "<tr><td class='style1'> Total Active Clients:</td><td class='style1'> " . $count;
...
$count = $row['COUNT(pd.pid)'];
	...
	echo "<tr><td class='style1'> Active Clients With No Assigned Provider:</td><td class='style1'> " . $count;
	
};

$query = "SELECT ".
 "COUNT(pd.pid), pd.pid, pd.DOB, pd.patient_active ".
 "FROM  `patient_data` AS pd ".
 "WHERE patient_active = 'YES' ".
// "AND providerID = '0' ".
 "AND DOB > (CURDATE() - INTERVAL 21 YEAR)"
 //"GROUP BY p.id"
 ;
...
$count = $row['COUNT(pd.pid)'];
	...
	echo "<tr><td class='style1'> Active Clients Under 21(Children):</td><td class='style1'> " . $count;
...
$query2 = "SELECT ".
 ...
 "WHERE patient_active = 'YES' ".
// "AND providerID = '0' ".
 "AND DOB < (CURDATE() - INTERVAL 20 YEAR)"
 //"GROUP BY p.id"
 ;
...
$count = $row['COUNT(pd.pid)'];
	...
	echo "<tr><td class='style1'> Active Clients 21 and Older(Adult):</td><td class='style1'> " . $count;
