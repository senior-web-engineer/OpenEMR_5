...
//		; 
$query = "SELECT ".
	 	"p.pid, p.lname, p.fname, p.program_0, p.program_1, p.providerID ".
	 				//",sp.pid, sp.servicecode, sp.units, sp.encounter, sp.date ".
					",fr.id, fr.form_id, fr.date, fr.encounter, COUNT(fr.form_name) ".
					",en.encounter, en.date ".
					//",SUM(sp.units) ".
					"FROM forms AS fr ".
					...
					//"JOIN forms AS fr ON fr.form_id = sp.id AND fr.pid = sp.pid ".
					"JOIN form_encounter AS en ON en.encounter = fr.encounter ".
			        "JOIN patient_data AS p ON p.pid = fr.pid ".
					//"WHERE sp.servicecode LIKE '$service_code' ".
			        //"AND fr.deleted = 0 ".
			        "WHERE fr.deleted = 0 ".
			        "AND fr.form_name = 'New Patient Encounter' ".
...
//					"AND sp.pid = '$patient_id' ".
					...
					"GROUP BY p.pid ".
					"ORDER BY COUNT(fr.form_name) DESC, p.lname"
			        ; 
...
while($row = mysql_fetch_array($result)){
			...
			$count = ($row['COUNT(fr.form_name)']);


	echo "<tr><td>".$row['pid']. "</td><td>". $row['lname']. ", ". $row['fname']. "<td> ". $count. "</td>". "</td><td>Unit(s) of service for ". $row['servicecode']. ", used for fiscal year <b>". substr("$_POST[fiscal_year]" ,0, 4). " :</b> </td> ". "<td><b>". $row['SUM(sp.units)']. "</td><td>". $units;
