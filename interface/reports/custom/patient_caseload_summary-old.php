...
<th class="filter-select">Last Name</th> 
	<th class="filter-select" style="width: 196px">First Name</th> <th class="filter-select">Count</th></tr></thead><tbody>
...
$query = "SELECT ".
 " p.id, p.fname, p.lname, pd.providerID, COUNT(pd.pid), pd.patient_active ".
 "FROM  `users` AS p ".
...
$count = $row['COUNT(pd.pid)'];
	...
	echo "<tr><td class='style1'> " .$lname."</td><td class='style1'> " . $fname."</td><td class='style1'> " . $count;
