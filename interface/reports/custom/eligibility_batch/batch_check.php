	...
	for ($i = 0; $i < count($_POST["pid"]); ++$i) {
		...
		if ($duplicate) {
			...
			if ($notauthorized == 'Unauthorized') {
				...
				//echo $login;
				$result1 = json_encode($result);
				//$result2 =	mysqli_real_escape_string($result);
				...
				$insurance_company = str_replace("'"," ",$insurance_company);
				...
				$sql = "INSERT INTO eligible (batch_id,pid,insurance_company,lname,fname,mname,sex,policy_number,active,plan_description,results_json)". 
						"VALUES ('$tracking_id','$pid','$insurance_company','$lname','$fname','$mname','$sex','$policy_number','$active','$plan_description','$result2')";
				...
				if (mysqli_query($mysqli, $sql)) {
					...
					//echo "<br>". $_POST["pid"][$i]. "<br>";
					echo "<br>New record created successfully. Last inserted ID is: ";
				} else {
					echo "<br>Error: " . $sql . "<br>" . mysqli_error($mysqli);
				}
