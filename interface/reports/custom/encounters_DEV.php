...
function generatePageElement($start, $pagesize, $billing, $issue, $text)
{
    ...
    $url="encounters_DEV.php?"."pagestart=".attr($start)."&"."pagesize=".attr($pagesize);
    $url.="&billing=".$billing;
    $url.="&issue=".$issue;
...
function changePageSize()
{
    ...
    pagesize=$(this).val();
    top.restoreSession();
    window.location.href="encounters_DEV.php?billing="+billing+"&issue="+issue+"&pagestart="+pagestart+"&pagesize="+pagesize;
}
...
?>
<?php if ($billing_view) { ?>
<a href='encounters_DEV.php?billing=0&issue=<?php echo $issue.$getStringForPage; ?>' onclick='top.restoreSession()' style='font-size:8pt'>(<?php echo htmlspecialchars(xl('To Clinical View'), ENT_NOQUOTES); ?>)</a>
<?php } else { ?>
<a href='encounters_DEV.php?billing=1&issue=<?php echo $issue.$getStringForPage; ?>' onclick='top.restoreSession()' style='font-size:8pt'>(<?php echo htmlspecialchars(xl('To Billing View'), ENT_NOQUOTES); ?>)</a>
<?php } ?>

<span style="float:right">
<!--added by dnunez 11/06/17 -->
	<a id="billing-filter" href="#">Click to color code billing status</a>
<!--end addition -->	
...
function searchPopup(){
    	if ($('#tooltipdiv').find('span.text:contains("Ready for Billing/Supervisor")').length > 0) {
	    $("#tooltipdiv").addClass("success").removeClass("error").removeClass("warning");
	}
	else if ($('#tooltipdiv').find('span.text:contains("Void")').length > 0) {
	    $("#tooltipdiv").addClass("error").removeClass("success").removeClass("warning");
	}
	else{
	    $("#tooltipdiv").addClass("warning").removeClass("error").removeClass("success");
	};
}
$(".encrow td div div").mouseover(function () {
	setTimeout(searchPopup, 400);
});
//Color filter for Billing
	$("#billing-filter").click(function() {
		$('.encrow td:nth-child(5), .encrow td:nth-child(3)').each(function(){
		    if ($(this).find('span').length > 0) {
		        $(this).parents('.encrow').addClass("success");
		    }
		    else {$(this).parents('.encrow').addClass("warning");
		    };
		});
	});
//end addition
