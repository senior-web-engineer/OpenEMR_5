...
switch ($api){
    ...
    case "getSelectorData": //params: id, where
        ...
        $res = sqlStatement("SELECT valueField, dataField, whereField, fromTable, setLimit, orderByField, customSelectFrom, version FROM openemr.form_textimporter_selectors where textImporterSelectorId = $selectorid;");
        $tiselectors = array();
        while($tiselector = SqlFetchArray($res)){
            $tiselectors['selectors'] = $tiselector;
        }
        ...
        if (count($tiselectors['selectors']) > 0){
            ...
            $fromTable = trim($tiselectors['selectors']['fromTable']);
            ...
            $customSelectFrom = $tiselectors['selectors']['customSelectFrom'];
            $version = $tiselectors['selectors']['version'];
            if ($version == null) {
                $version = 1;
            }
            if ($customSelectFrom != null && $version == 2){
                $queryString = " " . $customSelectFrom . " ";
            } else {
				$singleTable = $fromTable;
				$tables = explode(" ", trim($fromTable));
				if (sizeof($tables) > 2) {
					if (strtolower($tables[1]) == "as") {
						$singleTable = trim($tables[2]);
					} else {
						$singleTable = trim($tables[0]);
					}
				}
                $queryString = " SELECT " . $valueField . " AS 'value', " . $dataField . " AS 'desc', " . $singleTable . ".* FROM " . $fromTable . " ";
                if ($whereField != null){
            ...
            $message = str_replace("\n"," ",$queryString); //-------------!!!!!  DEBUG ONLY  !!!!!-----------: show query string in client
