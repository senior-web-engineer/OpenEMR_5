...
if (isset($_SESSION['whereto'])) {
 ...
 $attachment = getPatientAttachment($pid);
 $consentForm = getConsentForm($pid);

$consentFormCompleted = $consentForm ? $consentForm['completed'] ? true : false : false;
...
$(document).ready(function () {
    ...
    $("#consent-form-mental-health").load("./consentform/get_mental_health.php", {embeddedScreen: true}, function () {
        $("#consent-form-emotional-pain").load("./consentform/get_emotional_pain.php", {embeddedScreen: true}, function () {
            $("#consent-form-drug-alcohol").load("./consentform/get_drug.php", {embeddedScreen: true}, function () {
                $("#consent-form-physical-health").load("./consentform/get_physical_health.php", {embeddedScreen: true}, function () {
                    $("#consent-form-risk-assessment").load("./consentform/get_risk_assessment.php", {embeddedScreen: true}, function () {
                    });
                });
            });
        });
    ...
    $("#consent-form-treatment-plan").load("./consentform/get_treatment_plan.php", {embeddedScreen: true}, function () {
    });
    ...
    $('#avatarUpdateForm').on('submit', function(e){
        ...
        $.ajax({
            type: "POST",
            url: webRoot + '/portal/patient/api/patient/avatar',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
            success: function (response) {
                ...
                if(res.type == 'success'){
                    alert(res.message);
                    $('#openProfilePictureModal').modal('hide');
                } else {
                    alert(res.message);
                }
                console.log(res);
...
function editAppointment(mode,deid){
    ...
    <aside class="right-side">
        ...
        <div id="popwait" class="alert alert-warning" style="font-size:18px"><strong><?php echo xlt('Working!'); ?></strong> <?php echo xlt('Please wait...'); ?></div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="panel panel-primary collapse" id="reportpanel">
                        <header class="panel-heading"><?php echo xlt('My Reports'); ?>  </header>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="panel panel-primary collapse" id="profilepanel">
                        <header class="panel-heading"><?php echo xlt('My Profile'); ?></header>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="panel panel-primary collapse" id="mental-health-form">
                        <header class="panel-heading"><?php echo xlt('My Screening Forms'); ?></header>
                        <div id="consent-form-mental-health" class="panel-body"></div>
                        <div class="panel-footer"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="panel panel-primary collapse" id="emotional-pain-form">
                        <header class="panel-heading"><?php echo xlt('My Screening Forms'); ?></header>
                        <div id="consent-form-emotional-pain" class="panel-body"></div>
                        <div class="panel-footer"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="panel panel-primary collapse" id="drug-alcohol-form">
                        <header class="panel-heading"><?php echo xlt('My Screening Forms'); ?></header>
                        <div id="consent-form-drug-alcohol" class="panel-body"></div>
                        <div class="panel-footer"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="panel panel-primary collapse" id="physical-health-form">
                        <header class="panel-heading"><?php echo xlt('My Screening Forms'); ?></header>
                        <div id="consent-form-physical-health" class="panel-body"></div>
                        <div class="panel-footer"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="panel panel-primary collapse" id="risk-assessment-form">
                        <header class="panel-heading"><?php echo xlt('My Screening Forms'); ?></header>
                        <div id="consent-form-risk-assessment" class="panel-body"></div>
                        <div class="panel-footer"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="panel panel-primary collapse" id="treatment-plan-form">
                        <header class="panel-heading"><?php echo xlt('My Treatment Plan'); ?></header>
                        <div id="consent-form-treatment-plan" class="panel-body"></div>
                        <div class="panel-footer"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="panel panel-primary collapse" id="treatment-plan-detail-form">
                        <header class="panel-heading"><?php echo xlt('My Treatment Plan Detail'); ?></header>
                        <div id="treatment-plan-detail-panel" class="panel-body"></div>
                        <div class="panel-footer"></div>
                    </div>
                </div>
            </div>
...
<!-- Modal Start -->
<div class="modal fade" id="openProfilePictureModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">Upload Your Photo ID</h4>
            </div>
            <form role="form" id="avatarUpdateForm" action="" method="post" enctype="multipart/form-data">
                <div class="modal-body">
                    <div class="form-group">
                        <span>Looks like you have not uploaded your photo id yet !</span><br/>
                        <label for="code" style="color:red">Photo ID *</label>
                        <input type="file" name="attachment" class="form-control" id="attachment" onchange="readURL(this);" /><br/>
                        <img id="photo_id" src="avatar.png" alt="your image" style="width:250px;" />
                    </div>
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="patientId" class="form-control" value="<?php echo $pid ?>" />
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
...
<div id="guardianSignatureModal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            ...
            <div class="modal-body">
                <form name="signit" id="signit" class="sigPad">
                    ...
                    <button type="button" onclick="signConsentForm(this)"><?php echo xlt('Acknowledge as my Electronic Signature');?>.</button>
                </form>
            </div>
        </div>
        <div class="modal-footer">
