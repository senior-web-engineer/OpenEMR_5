...
class ApplicationTable
{
    ...
    public function portalNewAttachment($inputs)
    {
        $patientAttachment = sqlQuery("SELECT * FROM `patient_attachments` WHERE `pid`=?", array($inputs['pid']));
        if($patientAttachment){
            $updates = array($inputs['attachment'], date('Y-m-d H:i:s'));
            $updateQuery = "UPDATE patient_attachments set attachment='".$inputs['attachment']."', created_at='".date('Y-m-d H:i:s')."' where id=".$patientAttachment['id'];
            sqlInsertClean_audit($updateQuery, $updates);
        } else {
            $insertQuery = "INSERT INTO patient_attachments (`pid`, `attachment`, `created_at`)" .
                " VALUES ('".$inputs['pid'] . "','" . $inputs['attachment'] . "','" . date('Y-m-d H:i:s') ."')";
            sqlInsertClean_audit($insertQuery);
        }
        return true;
    }
}// app query class
